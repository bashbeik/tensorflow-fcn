...
def evaluate():
    ...
    test_images = feed_dict['images']
    test_labels = feed_dict['labels']
    test_logits = 4(test_images)
...
def train():
	...
	#lr = 1e-6
        lr = tf.train.exponential_decay(0.0,global_step,1000,0.9999,staircase = True)
        ...
        for step in range(max_steps):
            ...
            if step != 0 and step % 1000 == 0:
                ...
                summary_writer.add_summary(summary_str, step)
                 ...
                 #precision(cnt);
                 #test_loss = sess.run(evaluate())
                 #feed_dict  = reader.next_test()
                 #test_loss = sess.run(total_loss, feed_dict={images:feed_dict['images'], labels:feed_dict['labels']})
                 #format_str = ('%s: step %d, test loss = %.2f')
                 #print(format_str % (datetime.now(), step, test_loss))                 
